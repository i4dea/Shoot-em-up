...
////////////////AQUIIIIIIIIIII////////////////
    ...
    if (randNumber < 0.33 && this.game.time.now > this.lastPwpTime)
    {
        ...
        this.lastPwpTime = this.game.time.now + 5000;
    }
    else if (randNumber > 0.67 && this.game.time.now > this.lastPwpTime)
    {
    ...
    else if ( this.game.time.now > this.lastPwpTime)
    {
    ...
    update: function () {
      ...
      if (this.game.time.now > this.lastTimeRed) {
        var red = this.enemyred.create(Math.random()*600, -100, 'enemyred', 24, true);
        red.name = 'enemyred';
        red.body.velocity.y = +600;
        red.animations.add('fly');
        red.animations.play('fly', 24, true);
        this.lastTimeRed = this.game.time.now + 9000;
        red.events.onOutOfBounds.add(function () {
          if (red.y > this.player.y){
            this.resetGame();
          }
        }, this);
      }
      if (this.game.time.now > this.lastTimeBlue) {
        var blue = this.enemyblue.create(Math.random()*600, -100, 'enemyblue', 24, true);
        blue.name = 'enemyblue';
        blue.body.velocity.y = +400;
        blue.animations.add('fly');
        blue.animations.play('fly', 24, true);
        this.lastTimeBlue = this.game.time.now + 6000;
        blue.events.onOutOfBounds.add(function () {
          if (blue.y > this.player.y){
            this.resetGame();
          }
        }, this);
      }
      if (this.game.time.now > this.lastTimeGreegreen) {
        var green = this.enemygreen.create(Math.random()*600, -100, 'enemygreen', 24, true);
        green.name = 'enemygreen';
        green.body.velocity.y = +300;
        green.animations.add('fly');
        green.animations.play('fly', 24, true);
        this.lastTimeGreen = this.game.time.now + 1500;
        green.events.onOutOfBounds.add(function () {
          if (green.y > this.player.y){
            this.resetGame();
          }
        }, this);
      }
      if (this.game.time.now > this.lastTimeYellow) {
        var yellow = this.enemygreen.create(Math.random()*600, -100, 'enemyyellow', 24, true);
        yellow.name = 'enemyyellow';
        yellow.body.velocity.y = +500;
        yellow.animations.add('fly');
        yellow.animations.play('fly', 24, true);
        this.lastTimeYellow = this.game.time.now + 3000;
        yellow.events.onOutOfBounds.add(function () {
          if (yellow.y > this.player.y){
            this.resetGame();
          }
        }, this);
      }
